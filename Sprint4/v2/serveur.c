...
void * receivingFile_th(void * fileNameParam){
    ...
    dSCFile = accept(dSFile, (struct sockaddr*) &aC,&lg);
    if (dSCFile == -1){
...
int main(int argc, char *argv[]) {
	...
	adFile.sin_port = htons(atoi(arg1)+1);

	/*Nommage de la socket*/
	int bindRFile = bind(dSFile, (struct sockaddr*)&adFile, sizeof(adFile));
	if (bindRFile == -1){
	...
	int listenRFile = listen(dSFile, MAX_CLIENT);
	if (listenRFile == -1){
		perror("erreur au listen");
		exit(-1);
	}  

	/*Création de la socket pour la communication entre client*/
